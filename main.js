...
var playerData = {
	...
	crawlSpeedMultiple: crawlSpeedMultiple,
	areaLevel: areaLevel,
...
function saveData(){
	if(typeof(Storage) !== "undefined") {
		...
		localStorage.setItem("save",JSON.stringify(playerData));
		document.getElementById("test").innerHTML = "yes!";
} else {
    document.getElementById("test").innerHTML = "no support!";
}
...
function barUpdate(){
    ...
    document.getElementById("areaLabel").innerHTML='Area '+areaLevel+': '+progress+'%';
...
function encounter(){
    if(sentou==0){
        ...
        document.getElementById("progress").innerHTML = "Fighting hordes of goblins... ";
        tekiHP=tekiHP-myAtt;
        if(tekiHP<=0){
            sentou=0;
            tekiHP=0;
